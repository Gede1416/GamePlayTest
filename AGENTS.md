...
- `Assets/Scripts/Pipeline/Skill/`：技能管线（`SkillManager` 编排 / `Skill` 单条技能与额度 / `SkillPipeline` 三段接口与实现 / `SkillPipelineFactory` 枚举与装配）
...
| `Pipeline/Skill/SkillPipelineFactory.cs` | `SkillType` 枚举（Melee=近战范围1 / Ranged=远程范围3 / Buff=给自己挂 buff）+ 静态工厂：按技能配置造三段，和移动管线的 `PathPipelineFactory` 一个套路 | `CreateCastCheck(cfg)`（按 `cfg.onlyWhenHurt` 选普通 / 带血量判断）、`CreateTargetFinder(type, map)`、`CreateCaster(cfg)`、`Wire(skill, cfg, map)` |
| `Pipeline/Skill/SkillManager.cs` | **技能管理器**（挂在实体上，`[RequireComponent(typeof(Entity))]`）：手里是**一组技能**，`RunPipeline()` 挨个尝试（每个技能每回合最多放一次，额度在 `Skill` 里把关）；`Init()`/`Build()` 按预制体上配的 `skillCfgs` 造技能并调 `SkillPipelineFactory.Wire` 装三段；含 `[ContextMenu]` 跑一次 / 打印额度 | `List<SkillCfg> skillCfgs`（在预制体上配）、`MapManager map`、`IReadOnlyList<Skill> Skills`、`void Init()`（找 Entity + 造技能，由 `Entity.Init` 调）、`void Build()`（按配置重建，额度归零）、`bool RunPipeline()`、`void TickTurn()`（清"本回合放过" + 推各技能冷却）、`void AddDamage(float)`（转发给各技能，只有伤害类吃）、`void Clear()`、`List<Entity> targets` |
| `Pipeline/Skill/Skill.cs` | 一条技能（运行期）：三段 + **两条额度**（① 每个技能每回合最多放一次，② `oncePerBattle` 的一场一次）；`SkillCfg` 是它的配置（在预制体上配，数值当模板，运行期状态只在 Skill 里，清场重建不会把 buff 加成带过去） | `SkillCfg`（type / damage / cooldown / buff / oncePerBattle / onlyWhenHurt）、`Skill(SkillCfg)`、`ICastCheck CastCheck { get; set; }`、`ITargetFinder TargetFinder { get; set; }`、`ISkillCaster Caster { get; set; }`、`bool CanUse`、`bool UsedThisBattle`、`bool TryCast(Entity, List<Entity>)`（三段跑通才记额度）、`void TickTurn()`、`void AddDamage(float)` |
...
- 预制体（都在 `Assets/prefab/`，手工维护）：
  ...
  - `Melee.prefab`：Entity(uuid 2, 先攻 44, moveSteps 4) + AutoPilot(靠近, speed 5) + SkillManager(技能：近战 伤害 20 冷却 1；加攻击 buff（+5 × 3 回合）一场一次) + Health(team 0, 50) + DeathEffect(fadeTime 0.6) + BuffManager
  - `Ranger.prefab`：Entity(uuid 1, 先攻 10, moveSteps 3) + AutoPilot(远离, speed 5) + SkillManager(技能：远程 伤害 10 冷却 1；治疗 buff（回 10 × 3 回合，**血量没满才放**、不限制场次）) + Health(team 1, 50) + DeathEffect(fadeTime 0.6) + BuffManager
