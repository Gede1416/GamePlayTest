...
| `Events/BattleEvents.cs` | 各条消息的数据类（类型本身就是频道）：`TurnChangedEvent(round, totalRounds)`、`DamageEvent(target, amount)`、`HealthChangedEvent(entity, current, max)`、`EntityDiedEvent(entity, team)`、`EntitySpawnedEvent(entity)`、`BuffChangedEvent(entity)`、`SkillCastEvent(uuid, skillType)`、`BattleEndedEvent(winnerTeam)` | 每条消息只读字段 `round` / `totalRounds`、`target` / `amount`、`entity` / `current` / `max`、`entity` / `team`、`entity`、`entity`、`uuid` / `skillType`、`winnerTeam` |
...
| `Buffs/BuffManager.cs` | **Buff 管理器**（挂在实体上，一实体一个）：**只管自己身上这一队 buff**——`Queue<IBuff>` 存正在生效的 buff 零件；`TickTurn()` 轮转一圈——出队 → `Tick()` → 过期的 `Revert()` 且**不再入队**，没过期的转回队尾；**不认名字、也不造 buff**（没有 `CreateBuff`），收下的 buff 由技能那边 `new` 好递过来 | `void Init()`（记住主人 + 清队列，由 `Entity.Init` 调）、`void Clear()`（先把每条 `Revert()` 撤掉加成再清队列，由 `Entity.Clear` 调）、`void Add(IBuff)`（挂给谁 = 自己）、`void TickTurn()`、`int Count`、`IEnumerable<IBuff> Active`（只读给界面遍历）；挂上 / 到期 / 清场时广播 `BuffChangedEvent`；`[ContextMenu]` 有打印队列的调试入口 |
| `Buffs/BuffPipeline.cs` | **唯一的一条 buff 接口 `IBuff`**（效果零件自己就是一条 buff，队列里装的就是它）：`IsOver` / `Left` + `Init(target)`（挂给谁）+ `Apply()` / `Tick()` / `Revert()` 三步生命周期；**数值与持续回合都自己带**（构造函数注入）——持续回合按结算次数算，`<= 0` = 永久。没有 `BuffType` 枚举（buff 没有名字，界面显示零件类名），也没有第二条接口 | `IBuff.Init(Entity target)`（挂给谁，由 BuffManager 发下来，后面三个动作都作用在它身上）、`Left`（还剩几次结算，永久 = -1）/ `IsOver`、`Apply` / `Tick` / `Revert` |
| `Buffs/BuffEffect/HealEffect.cs` / `MoveStepsEffect.cs` / `AddAttackEffect.cs` | **三个效果零件 = 三条 buff**，一个文件一个：治疗（挂上 / 每次结算各回一次血）、加移动步数（Apply +N / Revert −N，走 `Entity.AddMoveSteps`）、加攻击力（Apply +N / Revert −N，走 `Entity.AddDamage`）；持续回合由构造函数给，自己记"已结算几次"（`Tick` 里 `elapsed++`），结算满就 `IsOver`；**每个动作都打一条日志**（目标名字 + 变化量 / 回血后的 Hp/MaxHp） | `HealEffect(float amount, int duration)`、`MoveStepsEffect(int steps, int duration)`、`AddAttackEffect(float amount, int duration)`，都实现 `IBuff`（自己带持续回合与挂给谁） |
...
- **Buff 只有一条接口，零件自己就是 buff**：`Buffs/` 只剩两样——管理器（`BuffManager`，**只管自己身上这一队 buff**：收下、排队、到期撤掉；不认名字也不造 buff）+ 零件（`BuffPipeline.cs` 里唯一的接口 `IBuff` + `BuffEffect/` 下三个实现，一个效果一个文件，数值与持续回合走构造函数）。**没有"组合 buff"那一层**（`Buff.cs` / `BuffType` / `IBuffEffect` 都已删）：队列里装的就是零件本身，所以零件自己回答"还剩几次结算 / 该不该结束"，`Apply` / `Tick` / `Revert` 三步各实现一遍。**这里没有 `Combination/` 目录**：挂哪个零件、持续几回合，是技能阶段三那个 caster 的常量（`HealBuffCaster` 等，一个 buff 一个 caster），Cast 时给每个目标 `new` 一个零件；挂给谁由技能阶段二挑好——没有 `BuffCfg` / `BuffFactory` 那套基础结构，也没有"按名字造 buff"（buff 没有名字，界面显示零件类名）。
