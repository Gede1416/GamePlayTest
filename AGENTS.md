...
| `Managers/MapManager.cs` | 网格 + **两份地图数据**（`int[,] cells` 格子→uuid 二维图、`Dictionary<int,Vector2Int> positions` uuid→位置，另有 `byUuid` uuid→实体），外部只读；`TryMove`/`CancelMove`/`SyncOccupied` 负责维护 | `Init()`（由 `BattleManager` 调：建网格 + 摆实体）、`Clear()`（清理：放掉地图数据与实体列表，保留 `spawnPoints` 配置）、`Build()`、`SyncOccupied()`（重建 + uuid 查重）、`ResetEntities()`、`UuidAt(cell)`、`EntityAt(cell)`、`EntitiesAt(cells)`、`EntityOf(uuid)`、`TryGetCell(uuid, out cell)`、`IsOccupied`、`CanEnter`、`InBounds`、`static Manhattan(a, b)`、`CellToWorld(col,row)` / `CellToWorld(cell)`、`WorldToCell`、`TryMove(from, step, out to)`、`CancelMove(from, to)`、`FindPath(from, to, path)`（四方向等权 BFS）、`SelfCheck()` / `SelfCheckPath()`（右键菜单自检）、常量 `Empty=0` / `Unknown=-1` |
...
- **事件走 `EventPipeline`**（静态收发器）：发送方 `Send(BattleEvent...)`，订阅方 `Subscribe(类型, 处理函数)`；目前只有 `Health.Die()` 发 `EntityDied`、`BattleManager.OnEntityDied` 收，结算后再发 `BattleEnded`。订阅在 `BattleManager.InitBattle()` 里订、`ClearBattle()` 里 `EventPipeline.Clear()` 清掉（静态表不能留旧订阅）。加新事件就在 `BattleEventType` 加一个值 + 在 `BattleEvent` 里加一个静态构造。
...
- `Health.team`：阵营就是个 int，没有仇恨表/友军保护；`Die()` 只是 `SetActive(false)` + 发消息，**尸体仍留在地图的 `entities` 列表里、格子照样占着**（要放开就在 `Die()` 里把它从地图登记里摘掉，或让 `MapManager.SyncOccupied()` 跳过阵亡者）
...
- **事件与结算** ✅ 新增基础事件管线 `EventPipeline`（`Send` / `Subscribe` / `Unsubscribe` / `Clear` + `BattleEvent` / `BattleEventType`）；`Health` 抽出 `Die()` 单独处理死亡并 `Send(EntityDied)`；`BattleManager` 收死亡消息、统计存活阵营，只剩一个（或全灭）就 `EndGame()` 让回合停手并发 `BattleEnded`
