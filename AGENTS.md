...
| `Pipeline/Skill/SkillPipeline.cs` | 技能管线三段接口（**都不依赖技能对象**，范围/目标数/伤害/挂哪种 buff 各自带）+ 实现：`CooldownCastCheck(cooldown)`（活着+冷却，顺带实现 `ICooldown`）、`MeleeTargetFinder(map)`（范围 1、1 目标）、`RangedTargetFinder(map)`（范围 3、1 目标）、`SelfTargetFinder()`（目标就是自己，给上 buff 的技能用）、`TargetPicker.Pick(...)`（两个攻击 finder 共用的挑选逻辑）、`DamageCaster(damage)`（扣血走 `Entity.TakeDamage` 门面）、`BuffCaster(buff)`（挂 buff 走 `Entity.AddBuff` 门面） | `ICastCheck.CanCast(caster)`、`ITargetFinder.TryFindTargets(caster, targets)`、`ISkillCaster.Cast(caster, targets)`、`ICooldown.StartCooldown()/TickTurn()` |
| `Pipeline/Skill/SkillPipelineFactory.cs` | `SkillType` 枚举（Melee=近战范围1 / Ranged=远程范围3 / Buff=给自己挂 buff）+ 静态工厂：按技能配置造三段，和移动管线的 `PathPipelineFactory` 一个套路 | `CreateCastCheck(cooldown)`、`CreateTargetFinder(type, map)`、`CreateCaster(cfg)`、`Wire(skill, cfg, map)` |
...
- 预制体（都在 `Assets/prefab/`，手工维护）：
  ...
  - `Ranger.prefab`：Entity(uuid 1, 先攻 10, moveSteps 3) + AutoPilot(远离, speed 5) + SkillManager(技能：远程 伤害 10 冷却 1；治疗 buff（回 10 × 3 回合）一场一次) + Health(team 1, 50) + DeathEffect(fadeTime 0.6) + BuffManager
