...
- `Attacker`：攻击管线**没有接进回合循环**（要放技能得自己调 `Attacker.RunPipeline()`，冷却要每回合调一次 `TickTurn()`）；目标获取从 `MapManager.entities` 里找，只认"挂了 `Entity` 且有 `Health`"的单位、按曼哈顿距离排序；`DamageCaster` 只扣血（没有击退/buff/动画表现）；远程没有视线/弹道判定（只看格子距离）；技能表现与碰触体那套 `SkillManager` + `Attack` 是两套并行机制
