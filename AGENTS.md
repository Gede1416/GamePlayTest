...
- `Assets/Scripts/Pipeline/Skill/`：技能管线（`SkillManager` 编排、Inspector 只配技能名 / `Skill` 单条技能与额度 / `SkillDefinition` 完整技能定义（三段 + 额度）/ `SkillPipeline` 三段接口与实现 / `SkillFactory` 技能名 → 定义）
...
| `Pipeline/Skill/SkillPipeline.cs` | 技能管线三段接口（**都不依赖技能对象**，范围/目标数/伤害/挂哪种 buff 各自带）+ 实现：`CooldownCastCheck(cooldown)`（活着+冷却，顺带实现 `ICooldown`，`CanCast` 是 virtual）、`WoundedCastCheck(cooldown)`（在它之上再加一条"自己的血量没满"——满血时治疗没意义，给治疗类技能用）、`MeleeTargetFinder(map)`（范围 1、1 目标）、`RangedTargetFinder(map)`（范围 3、1 目标）、`SelfTargetFinder()`（目标就是自己，给上 buff 的技能用）、`TargetPicker.Pick(...)`（两个攻击 finder 共用的挑选逻辑）、`DamageCaster(damage)`（扣血走 `Entity.TakeDamage` 门面）、`BuffCaster(buff)`（挂 buff 走 `Entity.AddBuff` 门面） | `ICastCheck.CanCast(caster)`、`ITargetFinder.TryFindTargets(caster, targets)`、`ISkillCaster.Cast(caster, targets)`、`ICooldown.StartCooldown()/TickTurn()` |
| `Pipeline/Skill/SkillFactory.cs` | **技能名枚举 + 技能定义表**：`SkillType`（Melee / Ranged / HealBuff / AttackBuff）+ 常量（各技能的伤害 / 冷却 / 挂哪种 buff）+ `Create(type, map)` 按技能名造一份完整定义。**想知道"这条技能长什么样"只看这里**；加技能 = 加一个枚举 + 一个 case，改数值 = 改这里的常量 | `static SkillDefinition Create(SkillType, MapManager)`（每次都是新实例；技能名没接实现返回 null 并警告） |
| `Pipeline/Skill/SkillDefinition.cs` | **一条技能的完整定义**：技能名 + 整条流水线（阶段一/二/三的实例）+ `oncePerBattle` 额度配置；`Run()` 依次跑三段并在放成后进冷却，是"效果"真正执行的地方 | `SkillType type`、`bool oncePerBattle`、`ICastCheck CastCheck` / `ITargetFinder TargetFinder` / `ISkillCaster Caster`、`float Damage`（伤害类技能才有，给日志 / 界面看）、`bool Run(Entity, List<Entity>)`、`void TickTurn()`、`void AddDamage(float)` |
| `Pipeline/Skill/SkillManager.cs` | **技能管理器**（挂在实体上，`[RequireComponent(typeof(Entity))]`）：手里是**一组技能**，`RunPipeline()` 挨个尝试（每个技能每回合最多放一次，额度在 `Skill` 里把关）；`Init()`/`Build()` 按预制体上配的 `skillTypes`（**只有技能名**）调 `SkillFactory.Create` 造技能；含 `[ContextMenu]` 跑一次 / 打印额度 | `List<SkillType> skillTypes`（在预制体上配，只选技能名）、`MapManager map`、`IReadOnlyList<Skill> Skills`、`void Init()`（找 Entity + 造技能，由 `Entity.Init` 调）、`void Build()`（按配置重建，额度归零）、`bool RunPipeline()`、`void TickTurn()`（清"本回合放过" + 推各技能冷却）、`void AddDamage(float)`（转发给各技能，只有伤害类吃）、`void Clear()`、`List<Entity> targets` |
| `Pipeline/Skill/Skill.cs` | 一条技能（运行期）：**只引用一份 `SkillDefinition`**（效果全交给它跑）+ **两条额度**（① 每个技能每回合最多放一次，② 定义里 `oncePerBattle` 的一场一次） | `Skill(SkillDefinition)`、`SkillDefinition Definition`、`SkillType Type`、`bool CanUse`、`bool UsedThisBattle`、`bool TryCast(Entity, List<Entity>)`（定义那边跑通才记额度）、`void TickTurn()`、`void AddDamage(float)` |
...
- 管线三段（移动：`ITargetSource` / `IPathPlanner` / `IPathExecutor`；技能：`ICastCheck` / `ITargetFinder` / `ISkillCaster`）的依赖都走**构造函数注入**，接口只收"这次要处理什么"（起点/终点/路径；施法者/目标列表）；不要往接口里塞 AutoPilot / MapManager / Entity。
...
- **数据所有权**：一份数据只有一个所有者。身份(uuid) / 先攻 / 回合步数 / 管线类型 归 `Entity`；生命值 + 阵营归 `Health`；本回合可走步数（`AutoPilot.ApplySteps`，只有"远离"挑落点用）、动画中（`AutoPilot.IsFollowing`）、冷却剩余与技能额度（各自的 `Skill`）属于各自组件的运行期状态。`Entity` 是对外唯一门面（`Hp` / `MaxHp` / `IsDead` / `Team` / `TakeDamage()` 转发），**不要把组件自己的属性搬进 Entity**（会变成两份真相 + 组件不能单独工作）。
...
- `SkillManager` / `Skill`：已接进回合（`Entity.TakeTurnRoutine()` 里放技能-移动-再放技能，回合开始 `TickTurn()` 清额度 + 推冷却）；**两条额度**——每个技能每回合最多放一次（`usedThisTurn`，放成了才记，所以空放不占名额，移动后还能补一次）、`oncePerBattle` 的一场一次（`usedThisBattle`，清场后按配置重建就归零，也就是**每场战斗各一次**，不是整局一次）；额度只看"放成没放成"，没有"驱散 / 免疫 / 沉默"这类拦截；治疗技能的"血量没满才放"是 `WoundedCastCheck` 写死在 `SkillFactory` 里的（原来是 `onlyWhenHurt` 开关），只看**施法者自己**的血量（队友 / 目标的血量条件没做），而且只判断"不满就放"——治疗量会不会溢出上限由 `Health.Heal` 自己截断；治疗技能解除了场次限制后，唯一刹车是"每回合一次 + 血量没满"，也就是**只要还伤着就每回合续一条治疗 buff**（同名 buff 会叠加，层数没有上限）；攻击目标从 `MapManager.entities` 里找、只认"挂了 `Entity` 且有 `Health`"的非己方且活着的单位、按曼哈顿距离排序；上 buff 的技能目标是自己（`SelfTargetFinder`），给队友 / 敌人的 buff 技能还没做；多个技能共用 `SkillManager.targets` 这一块目标缓冲（用完即覆盖）；`DamageCaster` 只扣血、`BuffCaster` 只挂 buff（没有击退 / 动画表现）；远程没有视线 / 弹道判定（只看格子距离）；近战 / 远程的范围是 `SkillPipeline.cs` 里的常量（现在 1 / 3）；技能的全部内容（范围 / 伤害 / 冷却 / 挂哪种 buff / 额度）由 `SkillFactory` 按技能名给出，**预制体上只配技能名**（`skillTypes` 列表）——想在 Inspector 里调数值 / 加中途 buff 这类自由都没了，要改就改 `SkillFactory`；`SkillDefinition` 每次 Create 都是新实例（冷却与伤害是运行期状态，不能跨实体共享）；旧的碰触体 `Attack` 与最早的技能管理器都已删除，技能只有技能管线一条路；界面上没有技能额度 / 冷却显示，只能靠 `[ContextMenu]` 打印或看日志
