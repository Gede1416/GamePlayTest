...
| `Buffs/BuffEffect.cs` | `IBuffEffect`：`Apply(Entity, BuffCfg)` / `Tick(Entity, BuffCfg)` / `Revert(Entity, BuffCfg)`——配置从参数进来，所以实现无状态。三个实现：`HealEffect`（Tick 回血）、`MoveStepsEffect`（Apply / Revert 加减步数）、`AttackEffect`（Apply / Revert 加减伤害） | 同左（"移除增加的 X"就是这个 `Revert`，不是单独类型） |
