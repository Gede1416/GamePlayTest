...
- `Assets/Scripts/Buffs/`：Buff（`BuffManager` 队列管理 / `Buff` 单条生命周期 / `BuffEffect` 三种效果 / `BuffFactory` 枚举 + 数值常量）
...
- `Assets/Scripts/Pipeline/Skill/`：技能管线（`SkillManager` 编排、Inspector 只配技能名 / `SkillPipeline` 三段接口 + 组合接口 + 零件接口 / `CastCheck/` 阶段一 释放判断 / `TargetFinder/` 阶段二 目标获取 / `SkillCaster/` 阶段三 技能释放 / `Combination/` 把三段拼成一条技能）
...
| `Events/BattleEvents.cs` | 各条消息的数据类（类型本身就是频道）：`TurnChangedEvent(round, totalRounds)`、`DamageEvent(target, amount)`、`HealthChangedEvent(entity, current, max)`、`EntityDiedEvent(entity, team)`、`EntitySpawnedEvent(entity)`、`BuffChangedEvent(entity)`、`SkillCastEvent(caster, skill)`、`BattleEndedEvent(winnerTeam)` | 每条消息只读字段 `round` / `totalRounds`、`target` / `amount`、`entity` / `current` / `max`、`entity` / `team`、`entity`、`entity`、`caster` / `skill`、`winnerTeam` |
...
| `Buffs/BuffManager.cs` | **Buff 管理器**（挂在实体上，一实体一个）：`Queue<Buff>` 存正在生效的 buff；`TickTurn()` 轮转一圈——出队 → `Trigger()` → 过期的 `Remove()` 且**不再入队**，没过期的转回队尾；数值 / 持续回合由 `BuffFactory` 决定，这里只管队列与生命周期 | `void Init()`（记住主人 + 清队列，由 `Entity.Init` 调）、`void Clear()`（先把每条 `Remove()` 撤掉加成再清队列，由 `Entity.Clear` 调）、`void Add(BuffType, List<Entity> targets = null)`（targets 留空 = 挂给自己）、`void TickTurn()`、`int Count`、`IEnumerable<Buff> Active`（只读给界面遍历）；挂上 / 到期 / 清场时广播 `BuffChangedEvent` |
| `Buffs/Buff.cs` | 一条 buff：`BuffCfg`（类型 / 数值 / 持续回合，**数值的唯一所有者**）+ `IBuffEffect` + 目标列表；三步生命周期 `Add()` / `Trigger()` / `Remove()`；持续回合按结算次数算（`duration <= 0` = 永久） | `BuffCfg(type, value, duration)`、`Buff(cfg, effect, targets)`、`void Add()`、`void Trigger()`（结算一次 + `elapsed++`）、`void Remove()`、`bool IsOver`、`int Left`（还剩几次结算，永久 = -1，给界面显示） |
| `Buffs/BuffEffect.cs` | `IBuffEffect`：`Apply(Entity, BuffCfg)` / `Tick(Entity, BuffCfg)` / `Revert(Entity, BuffCfg)`——配置从参数进来，所以实现无状态。三个实现：`HealEffect`（Tick 回血）、`MoveStepsEffect`（Apply / Revert 加减步数）、`AttackEffect`（Apply / Revert 加减伤害）；**每个动作都打一条日志**（目标名字 + 变化量 / 回血后的 Hp/MaxHp） | 同左（"移除增加的 X"就是这个 `Revert`，不是单独类型） |
| `Buffs/BuffFactory.cs` | `BuffType` 枚举（Heal / AddMoveSteps / AddAttack）+ 静态工厂：按类型造一条 `Buff`；**每种 buff 的数值与持续回合是这里的常量**（治疗 10 血 × 3 回合、步数 +2 × 3、攻击 +5 × 3，和攻击范围是常量一个套路） | `static Buff Create(BuffType, List<Entity>)`（类型没接实现就返回 null 并警告） |
...
| `Pipeline/Skill/SkillPipeline.cs` | 技能管线三段接口（**都不依赖技能对象**，范围 / 目标数 / 伤害 / 挂哪种 buff 各自带）+ 一条技能的组合接口 `ISkill`（三段 + `Init(map)` + `AddDamage`）+ 零件接口 `ISkillPart`（需要地图、或需要认"自己属于哪条技能"的零件实现它，组合技能在 `Init` 里把依赖发下去）+ 两个攻击 finder 共用的 `TargetPicker.Pick(...)` | `ISkill.Init(MapManager)` / `ISkill.AddDamage(float)`、`ICastCheck.CanCast(caster)`、`ITargetFinder.TryFindTargets(caster, targets)`、`ISkillCaster.Cast(caster, targets)`、`ISkillPart.Init(ISkill owner, MapManager map)` |
| `Pipeline/Skill/CastCheck/CooldownCastCheck.cs` | 阶段一「施法者活着 + 冷却好了」。**冷却自己记账**：收到 `SkillCastEvent`（自己那条技能放成了）进冷却、收到 `TurnChangedEvent`（进新回合）减一——所以**冷却 ≥ 1 顺带就是"每回合最多放一次"**；`CanCast` 是 virtual | `CooldownCastCheck(int cooldown = 0)`、`bool CanCast(caster)` |
| `Pipeline/Skill/CastCheck/WoundedCastCheck.cs` | 在它之上再加一条"**自己的血量没满**"（满血时治疗没意义），给治疗类技能用；血量读 `Entity.Hp` / `MaxHp` 门面 | 同上（继承 `CooldownCastCheck`） |
| `Pipeline/Skill/CastCheck/OnlyCastOnceCastCheck.cs` | **一场战斗只放一次**（例如加攻 buff）：同样靠 `SkillCastEvent` 自己记账；**一场 = 一个实例**——每场战斗技能按名单重建，这个标记跟着归零 | 同上 |
| `Pipeline/Skill/TargetFinder/MeleeTargetFinder.cs` / `RangedTargetFinder.cs` / `SelfTargetFinder.cs` | 阶段二：近战（范围 1 格、1 目标）/ 远程（范围 3 格、1 目标）/ 自己（目标就是自己，给上 buff 的技能用）；两个攻击 finder 实现 `ISkillPart` 拿地图 | `MeleeTargetFinder.Range` / `TargetCount`、`RangedTargetFinder.Range` / `TargetCount`、`bool TryFindTargets(caster, targets)` |
...
| `Pipeline/Skill/Combination/MeleeSkill.cs` 等四个 | **一条技能 = 三段零件拼起来**：`castChecks`（判断，与）/ `targetFinders`（目标，或）/ `skillCasters`（释放，与）；伤害 / 冷却 / 挂哪种 buff 是这里的常量。四个组合：`MeleeSkill`（伤害 20、冷却 1）、`RangedSkill`（伤害 10、冷却 1）、`HealBuffSkill`（血量没满才放 + 冷却 1 + 挂 Heal）、`AttackBuffSkill`（活着 + 一场一次 + 挂 AddAttack） | `Init(MapManager)`、`CanCast` / `TryFindTargets` / `Cast`、`AddDamage(float)`（只有伤害零件认） |
| `Pipeline/Skill/SkillManager.cs` | **技能管理器**（挂在实体上，`[RequireComponent(typeof(Entity))]`）：按预制体上的 `skillTypes`（**只有技能名**）调自己的 `CreateSkill` 造组合技能并装配（技能名 → 组合技能的映射就在这里，`SkillType` 枚举也在本文件）；`RunPipeline()` 挨个跑 **判断 → 目标 → 释放**，三段都过才算放成、并发 `SkillCastEvent`；**额度 / 冷却一行都不在这里**（归各技能的释放判断）；含 `[ContextMenu]` 跑一次 / 打印技能 | `List<SkillType> skillTypes`（预制体上配）、`MapManager map`、`IReadOnlyList<ISkill> Skills`、`void Init()`（找 Entity + 造技能，由 `Entity.Init` 调）、`void Build()`（按配置重建）、`bool RunPipeline()`、`void AddDamage(float)`（转发给各技能，只有伤害类吃）、`void Clear()`、`List<Entity> targets` |
...
- 管线三段（移动：`ITargetSource` / `IPathPlanner` / `IPathExecutor`；技能：`ICastCheck` / `ITargetFinder` / `ISkillCaster`）的依赖都走**构造函数注入**，接口只收"这次要处理什么"（起点/终点/路径；施法者/目标列表）；不要往接口里塞 AutoPilot / MapManager / Entity。技能这边是组合技能 new 出零件、装配时才把依赖发下去，所以零件要地图 / 要认自己那条技能就实现 `ISkillPart`（`Init(owner, map)`）。
- **技能 = 三段零件拼起来的组合**：一条技能在 `Combination/` 里把 `castChecks`（判断，**与**）/ `targetFinders`（目标，**或**）/ `skillCasters`（释放，**与**）拼起来，数值是组合类自己的常量；`ISkill` 就是把三段接口 + `Init` + `AddDamage` 合在一起的那张脸，`SkillManager` 只管按名单造、按顺序跑。
  **释放条件一律写成 `CastCheck`**（活着 / 冷却 / 血量没满 / 场次额度），不存在"管理器再补一条判断"；判断自己问不到的数据一律**走消息**：放成没放成靠 `SkillCastEvent`（`SkillManager.RunPipeline` 三段都过后发，检查收到后进冷却 / 记场次），回合推进靠 `TurnChangedEvent`（冷却减一）。**冷却 ≥ 1 就等于"每回合最多放一次"**（每大回合开头减一，一个单位一大回合只行动一次），所以没有单独的"每回合一次"检查。
...
- **事件走泛型 `EventPipeline`**：`Subscribe<T>(处理函数)` / `Unsubscribe<T>(...)` / `Send(new XxxEvent(...))`，按类型分发（`Dictionary<Type, Delegate>`），**加新事件只要在 `BattleEvents.cs` 加一个数据类，管线本身一行都不用改**。现在的收发关系：
  ...
  - `SkillCastEvent`：`SkillManager.RunPipeline` 三段都过（放成）后发 → 这条技能的 `CooldownCastCheck`（进冷却）/ `OnlyCastOnceCastCheck`（记"这场放过了"）
...
- **Buff**：数值 / 持续回合是 `BuffFactory` 里的常量（不能在 Inspector 调，要配表再说）；结算时机是**自己行动时**（`Entity.TakeTurnRoutine` 开头），不是"每大回合"，要改就挪到 `TurnChangedEvent` 那边；队列里**同一类型可以叠**（挂两次就两份，各自按自己的回合数过期），没有层数上限 / 驱散 / 免疫；"移除增加的移动步数 / 攻击力"是同一个效果的 `Revert`，**不是单独的 BuffType**（拆开就得各记一份数值，容易和实际加成对不上）；`Buff` 里存的是目标列表（`Add(type, targets)` 可以挂给别人），现在只用到"挂给自己"；尸体上的 buff 仍会随它自己的回合结算（`TurnManager` 不看 `IsDead`），但治疗对它无效（`Health.Heal` 遇到已阵亡直接返回）；清场时 `BuffManager.Clear()` 会把加成撤掉，所以别在外面直接改 `moveSteps` / `damage` 记"临时加成"，走 buff
...
- `SkillManager` / 技能：已接进回合（`Entity.TakeTurnRoutine()` 里放技能-移动-再放技能，第二遍只补放"走完才够得着"的那次，放成过的会被各自的释放判断挡住）；**额度 / 冷却全在释放判断里**——冷却（`CooldownCastCheck`，≥ 1 顺带就是每回合一次）、一场一次（`OnlyCastOnceCastCheck`）、血量没满（`WoundedCastCheck`），都是收到 `SkillCastEvent` / `TurnChangedEvent` 自己记账，管理器一条判断都不补；额度只看"放成没放成"（空放不占名额，移动后还能补一次），**没有"驱散 / 免疫 / 沉默"这类拦截**；治疗只看**施法者自己**的血量（队友 / 目标的血量条件没做），而且只判断"不满就放"——治疗量会不会溢出上限由 `Health.Heal` 自己截断；治疗不限场次，唯一刹车是"冷却 1 + 血量没满"，也就是**只要还伤着就每回合续一条治疗 buff**（同名 buff 会叠加，层数没有上限）；攻击目标从 `MapManager.entities` 里找、只认"挂了 `Entity` 且有 `Health`"的非己方且活着的单位、按曼哈顿距离排序；上 buff 的技能目标是自己（`SelfTargetFinder`），给队友 / 敌人的 buff 技能还没做；多个技能共用 `SkillManager.targets` 这一块目标缓冲（用完即覆盖）；`DamageCaster` 只扣血、`BuffCaster` 只挂 buff（没有击退 / 动画表现）；远程没有视线 / 弹道判定（只看格子距离）；近战 / 远程的范围是各自 finder 里的常量（现在 1 / 3），伤害 / 冷却 / 挂哪种 buff 是各自组合技能里的常量——**预制体上只配技能名**（`skillTypes` 列表），想在 Inspector 里调数值 / 加中途 buff 这类自由都没有，要改就改组合类；四个组合类的三段聚合循环是**重复的**（要不要抽个基类再说）；释放判断是订阅消息记账、**自己不退订**（靠 `BattleManager.ClearBattle()` 末尾的 `EventPipeline.Clear()` 兜，所以同一场战斗里重复调 `SkillManager.Build()` 会订两遍）；旧的碰触体 `Attack` 与最早的技能管理器都已删除，技能只有技能管线一条路；界面上没有技能额度 / 冷却显示，只能靠 `[ContextMenu]` 打印或看日志
...
- **技能结构重构** ✅ **删掉 `Skill` / `SkillDefinition` / `SkillFactory` 这套基础结构**：一条技能就是 `Combination/` 里的一个组合类（`MeleeSkill` / `RangedSkill` / `HealBuffSkill` / `AttackBuffSkill`）——三段零件列表 + 数值常量，`CanCast` / `TryFindTargets` / `Cast` 分别取 与 / 或 / 与；三段实现拆进 `CastCheck/` / `TargetFinder/` / `SkillCaster/` 三个目录，装配时靠 `ISkillPart.Init(owner, map)` 把"自己是谁"和地图发下去（`MeleeTargetFinder` 原来那个没接上的 `_map` 就是这么修好的）；`SkillManager` 只按 `skillTypes` 造组合技能、跑三段、放成后发 `SkillCastEvent`（技能名 → 组合技能的映射收进它自己的 `CreateSkill`）；**所有释放条件都成了 `CastCheck`**（活着 / 冷却 / 血量没满 / 一场一次），`ICooldown` 随之删掉——冷却改由 `CooldownCastCheck` 收消息推进，`Entity.TakeTurnRoutine` 里的 `Skills.TickTurn()` 一起去掉
