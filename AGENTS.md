...
- `Assets/Scripts/Buffs/`：Buff（`BuffManager` 只管自己的队列 / `Buff` 一条运行期缓冲 / `BuffPipeline` 两条接口 + `BuffType` 名字 / `BuffEffect/` 效果零件）；**"挂哪种 buff"由技能的阶段三配**（`Pipeline/Skill/SkillCaster/*BuffCaster.cs`）
...
| `Buffs/BuffManager.cs` | **Buff 管理器**（挂在实体上，一实体一个）：**只管自己身上这一队 buff**——`Queue<IBuff>` 存正在生效的 buff；`TickTurn()` 轮转一圈——出队 → `Trigger()` → 过期的 `Remove()` 且**不再入队**，没过期的转回队尾；**不认名字、也不造 buff**（没有 `CreateBuff`），收下的 buff 由技能那边造好递过来 | `void Init()`（记住主人 + 清队列，由 `Entity.Init` 调）、`void Clear()`（先把每条 `Remove()` 撤掉加成再清队列，由 `Entity.Clear` 调）、`void Add(IBuff)`（挂给谁 = 自己）、`void TickTurn()`、`int Count`、`IEnumerable<IBuff> Active`（只读给界面遍历）；挂上 / 到期 / 清场时广播 `BuffChangedEvent`；`[ContextMenu]` 有打印队列的调试入口 |
| `Buffs/BuffPipeline.cs` | `BuffType` 枚举（Heal / AddMoveSteps / AddAttack，**只是 buff 的名字**，给界面与日志看，没有"按名字造 buff"）+ 两条接口：`IBuff`（`Type` / `Left` / `IsOver` + `Init(target)` + `Add` / `Trigger` / `Remove`）与 `IBuffEffect`（效果零件：`Apply(target)` / `Tick(target)` / `Revert(target)`，**数值自己带**，构造函数注入） | `IBuff.Init(Entity target)`（挂给谁，由 BuffManager 发下来）、`Type` / `Left`（还剩几次结算，永久 = -1）/ `IsOver`、`IBuffEffect.Apply` / `Tick` / `Revert` |
...
| `Buffs/Buff.cs` | **一条运行期 buff**：效果零件 + 名字 + 持续回合 + 挂给谁 + 已结算次数；三步生命周期就是把零件的三个动作对目标跑一遍。**内容由技能的阶段三给**（`new Buff(type, duration, effects...)`），BuffManager 收下时包成它——所以没有任何"按类型"的 buff 类 | `Buff(BuffType, int duration, params IBuffEffect[])`、`Init(target)`、`Add()` / `Trigger()` / `Remove()`、`BuffType Type`、`int Left`、`bool IsOver` |
...
| `Pipeline/Skill/SkillCaster/DamageCaster.cs` / `HealBuffCaster.cs` / `MoveStepsBuffCaster.cs` / `AddAttackBuffCaster.cs` | 阶段三：扣血（走 `Entity.TakeDamage` 门面，`damage` 能被 buff 加减）/ **挂 buff——一个 buff 一个 caster**（`HealBuffCaster` 等），"这条技能挂的是什么 buff"（效果零件 + 名字 + 持续回合）就在这个类的常量里，Cast 时给每个目标 `new` 一条 `Buff`（每次新建：buff 带自己的结算次数） | `DamageCaster(float damage)`、`AddDamage(float)`、`XxxBuffCaster()`、`bool Cast(caster, targets)` |
...
- **Buff 的"组合"归技能**：`Buffs/` 只剩三样——管理器（`BuffManager`，**只管自己身上这一队 buff**：收下、排队、到期撤掉；不认名字也不造 buff）+ 接口文件（`BuffPipeline.cs`，`BuffType` 与 `IBuff` / `IBuffEffect` 在这里，`BuffType` 只当 buff 的名字）+ 零件目录（`BuffEffect/`，一个效果一个文件，数值走构造函数）；**运行期对象是 `Buff.cs`**（一条 buff = 效果零件 + 名字 + 持续回合 + 挂给谁 + 已结算次数，`Add` / `Trigger` / `Remove` 三步就是把零件的三个动作对目标跑一遍）。**这里没有 `Combination/` 目录**：一条 buff 带哪些效果、持续几回合，是技能阶段三那个 caster 的常量（`HealBuffCaster` 等，一个 buff 一个 caster），Cast 时给每个目标 `new` 一条 `Buff`；挂给谁由技能阶段二挑好——没有 `BuffCfg` / `BuffFactory` 那套基础结构，也没有"按名字造 buff"。
...
- **Buff**：数值 / 持续回合是技能阶段三那个 caster 里的常量（不能在 Inspector 调，要配表再说；一个 buff 一个 caster，加起来就是"组合 buff"）；结算时机是**自己行动时**（`Entity.TakeTurnRoutine` 开头），不是"每大回合"，要改就挪到 `TurnChangedEvent` 那边；队列里**同一类型可以叠**（挂两次就两份，各自按自己的回合数过期），没有层数上限 / 驱散 / 免疫；"移除增加的移动步数 / 攻击力"是同一个效果零件的 `Revert`，**不是单独的 BuffType**（拆开就得各记一份数值，容易和实际加成对不上）；一条 buff 只挂一个实体（没有目标列表了），要挂给谁由技能阶段二挑、阶段三逐个挂；尸体上的 buff 仍会随它自己的回合结算（`TurnManager` 不看 `IsDead`），但治疗对它无效（`Health.Heal` 遇到已阵亡直接返回）；清场时 `BuffManager.Clear()` 会把加成撤掉，所以别在外面直接改 `moveSteps` / `damage` 记"临时加成"，走 buff；三步生命周期只有 `Buff` 这一份实现（`Combination/` 删掉后不再有重复的组合类）
...
- **未做**：Map 配置对象（用户说暂时不用）。
- 待定（问过用户、还没定）：技能组合类的装配 / 聚合循环是五个组合类各抄一遍的，要不要抽基类；见「已知缺口」。（Buff 那边已经没有组合类：一条 buff 就是 `Buff` 一个类，三步生命周期只有一份实现）
