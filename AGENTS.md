...
- `Assets/Scripts/Buffs/`：Buff（`BuffManager` 只管自己的队列 / `BuffPipeline` 只有一条接口 `IBuff` / `BuffEffect/` 效果零件——**零件自己就是一条 buff**）；**"挂哪种 buff"由技能的阶段三配**（`Pipeline/Skill/SkillCaster/*BuffCaster.cs`）
- `Assets/Scripts/Events/`：事件（`EventPipeline` 泛型收发 + `BattleEvents` 里各条消息的数据类）
- `Assets/Scripts/UI/`：战斗界面（`BattleUIManager` 总管 + `HealthBar` 血条 + `DamagePopup` 伤害数字；**buff 条已移除**，见「已知缺口」）
- `Assets/Scripts/Pipeline/Movement/`：移动管线（`AutoPilot` / `PathPipeline` / `TargetSources` / `PathPipelineFactory`）
- `Assets/Scripts/Pipeline/Skill/`：技能管线（`SkillManager` 编排、Inspector 只配技能名 + 放成后发 SkillCastEvent / `SkillPipeline` 三段接口 + 组合接口 / `CastCheck/` 阶段一 释放判断 / `TargetFinder/` 阶段二 目标获取 / `SkillCaster/` 阶段三 技能释放 / `Combination/` 把三段拼成一条技能）
...
| `Events/BattleEvents.cs` | 各条消息的数据类（类型本身就是频道）：`TurnChangedEvent(round, totalRounds)`、`DamageEvent(target, amount)`、`HealthChangedEvent(entity, current, max)`、`EntityDiedEvent(entity, team)`、`EntitySpawnedEvent(entity)`、`SkillCastEvent(uuid, skillType)`、`BattleEndedEvent(winnerTeam)` | 每条消息只读字段 `round` / `totalRounds`、`target` / `amount`、`entity` / `current` / `max`、`entity` / `team`、`entity`、`uuid` / `skillType`、`winnerTeam` |
| `UI/BattleUIManager.cs` | **战斗界面总管**：全靠事件驱动——回合数（`TurnChangedEvent` 写场景 Canvas 上的文字）、伤害数字（`DamageEvent` 在被打实体 UI 点位下**实例化预制体**）、胜方（`BattleEndedEvent`）、血条（`EntitySpawnedEvent` 时给实体**实例化血条预制体**）。界面元素本身都是场景 Canvas / 预制体，这里只加载、实例化、销毁，**不在代码里组装 UI** | `void Init()`（加载预制体 + 订事件，由 `BattleManager` 调）、`void Clear()`（退订 + 销毁实例化出来的血条 / 伤害数字）、`TMP_Text roundText` / `TMP_Text resultText`（场景 Canvas 下的两个 TMP 文字）、`string healthBarPath` / `string damagePopupPath`（预制体路径） |
...
| `Buffs/BuffManager.cs` | **Buff 管理器**（挂在实体上，一实体一个）：**只管自己身上这一队 buff**——`Queue<IBuff>` 存正在生效的 buff 零件；`TickTurn()` 轮转一圈——出队 → `Tick(owner)` → 过期的 `Revert(owner)` 且**不再入队**，没过期的转回队尾；**不认名字、也不造 buff**（没有 `CreateBuff`），收下的 buff 由技能那边 `new` 好递过来 | `void Init()`（记住主人 + 清队列，由 `Entity.Init` 调）、`void Clear()`（先把每条 `Revert(owner)` 撤掉加成再清队列，由 `Entity.Clear` 调）、`void Add(IBuff)`（作用在谁身上 = 自己，当场 `Apply(owner)`）、`void TickTurn()`；`[ContextMenu]` 有打印队列的调试入口 |
| `Buffs/BuffPipeline.cs` | **唯一的一条 buff 接口 `IBuff`**（效果零件自己就是一条 buff，队列里装的就是它）：`IsOver` + `Apply(entity)` / `Tick(entity)` / `Revert(entity)` 三步生命周期；**挂给谁不存起来**——目标每次都当参数传进来；**数值与持续回合都自己带**（构造函数注入），持续回合按结算次数算，`<= 0` = 永久。没有 `BuffType` 枚举、没有第二条接口 | `IBuff.IsOver`、`IBuff.Apply(Entity entity)` / `Tick(Entity entity)` / `Revert(Entity entity)` |
| `Buffs/BuffEffect/HealEffect.cs` / `MoveStepsEffect.cs` / `AddAttackEffect.cs` | **三个效果零件 = 三条 buff**，一个文件一个：治疗（挂上 / 每次结算各回一次血）、加移动步数（Apply +N / Revert −N，走 `Entity.AddMoveSteps`）、加攻击力（Apply +N / Revert −N，走 `Entity.AddDamage`）；持续回合由构造函数给，自己记"已结算几次"（`Tick` 里 `elapsed++`），结算满就 `IsOver`；**每个动作都打一条日志**（目标名字 + 变化量 / 回血后的 Hp/MaxHp） | `HealEffect(float amount, int duration)`、`MoveStepsEffect(int steps, int duration)`、`AddAttackEffect(float amount, int duration)`，都实现 `IBuff`（自己带持续回合；目标由调用方按参数传进来，零件不记） |
...
| `Pipeline/Skill/SkillPipeline.cs` | 技能管线三段接口（**都不依赖技能对象、也不存外部依赖**，范围 / 目标数 / 伤害 / 挂哪种 buff 各自带）+ 一条技能的组合接口 `ISkill`（三段 + `Type` + `AddDamage`）+ 两个攻击 finder 共用的 `TargetPicker.Pick(...)`（返回目标列表） | `ISkill.Type` / `ISkill.AddDamage(float)`、`ICastCheck.CanCast(caster)`、`ITargetFinder.TryFindTargets(caster, map)`（**地图每次调用传进来**）、`ISkillCaster.Cast(caster, targets)` |
| `Pipeline/Skill/CastCheck/CooldownCastCheck.cs` | 阶段一「施法者活着 + 冷却好了」。**"放成没放成"靠消息**：订 `SkillCastEvent`，比对自己的施法者 id（第一次 `CanCast` 时从 `caster.Uuid` 记下）与技能名，对上了才进冷却；收 `TurnChangedEvent` 每大回合减一——所以**冷却 ≥ 1 顺带就是"每回合最多放一次"**；订阅在构造函数里做 | `CooldownCastCheck(SkillType, int cooldown = 0)`、`bool CanCast(caster)` |
...
| `Pipeline/Skill/SkillCaster/DamageCaster.cs` / `HealBuffCaster.cs` / `MoveStepsBuffCaster.cs` / `AddAttackBuffCaster.cs` | 阶段三：扣血（走 `Entity.TakeDamage` 门面，`damage` 能被 buff 加减）/ **挂 buff——一个 buff 一个 caster**（`HealBuffCaster` 等），"这条技能挂的是什么 buff"（挂哪个零件 + 数值 + 持续回合）就在这个类的常量里，Cast 时给每个目标 `new` 一个零件（每次新建：零件自己带结算次数） | `DamageCaster(float damage)`、`AddDamage(float)`、`XxxBuffCaster()`、`bool Cast(caster, targets)` |
| `Pipeline/Skill/Combination/MeleeSkill.cs` 等五个 | **一条技能 = 三段零件拼起来**：`castChecks`（判断，与）/ `targetFinders`（目标，或）/ `skillCasters`（释放，与）；技能名 / 伤害 / 冷却 / 挂哪种 buff 是这里的常量。五个组合：`MeleeSkill`（伤害 20、冷却 1）、`RangedSkill`（伤害 10、冷却 1）、`HealBuffSkill`（冷却 + 血量没满，两条判断的"与" + 挂 Heal）、`AttackBuffSkill`（活着 + 一场一次 + 挂 AddAttack）、`MoveStepsBuffSkill`（活着 + 冷却 + 挂 AddMoveSteps） | `SkillType Type`、`CanCast` / `TryFindTargets(caster, map)` / `Cast`、`AddDamage(float)`（只有伤害零件认） |
...
- `BattleManager`（GO `321782083`）：`mapPath` = `Assets/prefab/map1.prefab`、`turnPath` = `Assets/prefab/turn1.prefab`、
  ...
  `mapPos` = `map_pos`、`uiManager` = 同一个物体上的 `BattleUIManager`（接好 `roundText` / `resultText` 指向 BattleCanvas 下的两个 TMP 文字，预制体路径 `Assets/prefab/HealthBar.prefab` 与 `Assets/prefab/DamagePopup.prefab`）
...
- **技能 = 三段零件拼起来的组合**：一条技能在 `Combination/` 里把 `castChecks`（判断，**与**）/ `targetFinders`（目标，**或**）/ `skillCasters`（释放，**与**）拼起来，技能名 / 数值是组合类自己的常量；`ISkill` = 三段接口 + `Type` + `AddDamage`，`SkillManager` 只管按名单造、按顺序跑。
  **释放条件一律写成 `CastCheck`**（活着 / 冷却 / 血量没满 / 场次额度），不存在"管理器再补一条判断"，检查之间也**不互相继承**（`WoundedCastCheck` 只管血量），要哪几条就在组合类的 `castChecks` 列表里与起来；**"放成没放成"这条判断拿不到的数据靠消息**：`SkillManager` 放成后发 `SkillCastEvent`（施法者 uuid + 技能名），检查内部缓存自己的技能名（构造函数）与施法者 id（第一次 `CanCast` 时从 `caster.Uuid` 记下），收到消息两个都对上才记自己的账——`OnlyCastOnceCastCheck` 标记"这场放过了"，`CooldownCastCheck` 置冷却，冷却每大回合收 `TurnChangedEvent` 减一。**冷却 ≥ 1 就等于"每回合最多放一次"**（一个单位一大回合只行动一次），所以没有单独的"每回合一次"检查。
- **Buff 只有一条接口，零件自己就是 buff**：`Buffs/` 只剩两样——管理器（`BuffManager`，**只管自己身上这一队 buff**：收下、排队、到期撤掉；不认名字也不造 buff）+ 零件（`BuffPipeline.cs` 里唯一的接口 `IBuff` + `BuffEffect/` 下三个实现，一个效果一个文件，数值与持续回合走构造函数）。**没有"组合 buff"那一层**（`Buff.cs` / `BuffType` / `IBuffEffect` 都已删）：队列里装的就是零件本身，所以零件自己回答"该不该结束"（`IsOver`），`Apply` / `Tick` / `Revert` 三步各实现一遍；**目标不存字段**——`BuffManager` 每次把主人当参数传下去（和技能零件"不存地图"一个套路）。**这里没有 `Combination/` 目录**：挂哪个零件、持续几回合，是技能阶段三那个 caster 的常量（`HealBuffCaster` 等，一个 buff 一个 caster），Cast 时给每个目标 `new` 一个零件；挂给谁由技能阶段二挑好——没有 `BuffCfg` / `BuffFactory` 那套基础结构，也没有"按名字造 buff"（buff 没有名字，日志只打零件类名）。
...
- **事件走泛型 `EventPipeline`**：`Subscribe<T>(处理函数)` / `Unsubscribe<T>(...)` / `Send(new XxxEvent(...))`，按类型分发（`Dictionary<Type, Delegate>`），**加新事件只要在 `BattleEvents.cs` 加一个数据类，管线本身一行都不用改**。现在的收发关系：
  ...
  - `BattleEndedEvent`（EndGame）：UI 显示胜方
  - `SkillCastEvent`（SkillManager 放成后发，带施法者 uuid + 技能名）：各技能的 `CooldownCastCheck` / `OnlyCastOnceCastCheck` 对上 id 与技能名后记账
...
- **Buff**：数值 / 持续回合是技能阶段三那个 caster 里的常量（不能在 Inspector 调，要配表再说；一个 caster 对应一个零件）；结算时机是**自己行动时**（`Entity.TakeTurnRoutine` 开头），不是"每大回合"，要改就挪到 `TurnChangedEvent` 那边；队列里**同类零件可以叠**（挂两次就两份，各自按自己的回合数过期），没有层数上限 / 驱散 / 免疫；"移除增加的移动步数 / 攻击力"是同一个零件的 `Revert`（拆开就得各记一份数值，容易和实际加成对不上）；一条 buff 只挂一个实体（没有目标列表了），要挂给谁由技能阶段二挑、阶段三逐个挂；尸体上的 buff 仍会随它自己的回合结算（`TurnManager` 不看 `IsDead`），但治疗对它无效（`Health.Heal` 遇到已阵亡直接返回）；清场时 `BuffManager.Clear()` 会把加成撤掉，所以别在外面直接改 `moveSteps` / `damage` 记"临时加成"，走 buff；**一份 buff 只能是一个零件**（`Buff.cs` 那一层删掉后没法把两个效果拼成一条，要拼就得再引入一个"装多个 `IBuff`"的零件）；"还剩几次结算"这套记账（`duration` / `elapsed` / `IsOver`）在三个零件里各抄了一遍；零件没有名字（`BuffType` 已删），日志只能打零件类名；`IBuff` 上没有"还剩几次"这种查询（`Left` 已删），要看剩余只能在各自零件里加
- **Buff 界面**：**暂时没有**（只有功能、没有表现）——buff 条 `UI/BuffBar.cs` 与 `Assets/prefab/BuffBar.prefab` 已删，`buffBarPath`、`BuffChangedEvent`（它唯一的订阅者就是 buff 条）与 `BattleUIManager` 里挂 buff 条的那段也一并去掉；`BuffManager` 上的 `Count` / `Active` 当初也只给 buff 条遍历用，按"没人用就删"一起去掉了。以后要重新显示 buff，得自己再挑一个做法（事件 / 轮询 + 只读入口）
...
- `SkillManager` / 技能：已接进回合（放技能 → 移动 → 再放技能，第二遍只补放走完才够得着的那次）。**额度 / 冷却全在释放判断里**（冷却 ≥ 1 顺带就是每回合一次、一场一次、血量没满），判据是 **`SkillCastEvent`**：管理器三段都过后发一条（带施法者 uuid + 技能名），判断自己缓存技能名、第一次 `CanCast` 时从 `caster.Uuid` 记下施法者 id，两个都对上才记账——所以**施法者 id 必须被 `CanCast` 问过一次**（管线正常跑一定先问）。没有"驱散 / 免疫 / 沉默"拦截；同一实体配两条同名技能时，两条判断的 id 与技能名会同时对上（都会记账），要区分得给技能加实例 id。治疗只看施法者自己的血量、只判"不满就放"（溢出由 `Health.Heal` 截断），不限场次 → **只要还伤着就每回合续一条治疗 buff**（同名会叠、无上限）。攻击目标来自 `MapManager.entities`（非己方且活着，按曼哈顿距离排序），阶段二每次**新造一个目标列表**；上 buff 的技能目标是自己，给队友 / 敌人的 buff 技能没做；远程没有视线 / 弹道判定（只看格子距离）；范围 / 伤害 / 冷却 / buff 种类都是各自零件与组合类的常量，**预制体上只配技能名**；两个检查都在构造函数里订 `SkillCastEvent` / `TurnChangedEvent` 且**不退订**（靠 ClearBattle 末尾的 `EventPipeline.Clear()` 兜，同一场里重复 `Build()` 会订两遍、冷却一次减二）；组合类的聚合循环是重复的（要不要抽基类待定）；界面上没有技能额度 / 冷却显示，只能靠 `[ContextMenu]` 打印或看日志
...
- **界面（UI）**：文字全用 TMP（HUD 两个文字 + 两个按钮标签 + 伤害数字）；**默认字体 `LiberationSans SDF` 没有中文字形，界面文字只能英文 / 数字**（`Round x/y` / `Team 0 Wins` / `Draw` / `Start` / `Rebuild`），要中文得自己做带中文字形的 TMP 字体并在 TMP Settings 设回退；TMP Essential Resources 已入库，缺了文字不显示；血条挂在实体 UI 点位下（世界空间 Canvas + 内置 UISprite），每帧转向 `Camera.main`（**场景里没主相机就看不到**）；血条没有缓动 / 数字，伤害数字不合并同帧多次、不加暴击前缀；开始 / 重开按钮点完不会自己隐藏（重复点是安全的）
- 脚本都还没在播放模式下跑过（两个角色都没有 Rigidbody，移动是直接写 `transform.position`）

## 重构现状

- 各层都是一个套路：**管理器 + 三段零件 + 组合类**（实体 / 地图 / 回合 / 移动管线 / 技能管线 / UI / 事件总线都已摆好；**Buff 没有组合类**——队列里装的就是效果零件本身，接口只有 `IBuff` 一条）。
  每一步怎么改的、为什么这么改，看 `git log` 与提交信息（本项目约定一个改动一个提交）。
- **未做**：Map 配置对象（用户说暂时不用）。
- 待定（问过用户、还没定）：技能组合类的装配 / 聚合循环是五个组合类各抄一遍的，要不要抽基类；见「已知缺口」。（Buff 那边已经没有组合类：队列里装的就是效果零件本身，三步生命周期在每个零件里各实现一遍）
