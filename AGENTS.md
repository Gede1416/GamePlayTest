...
- `Assets/Scripts/Events/`：事件管线（`EventPipeline`：事件类型 / 事件数据 / 收发）
...
| `Managers/BattleManager.cs` | **战场管理**。`BuildBattle()` 按三步拆开：**① 清理资源 `ClearBattle()` → ② 资源加载 + 绑定 + 组件缓存 `LoadBattle()` → ③ 初始化 `InitBattle()`**。`LoadBattle()` 按 Inspector 里配的路径（`mapPath` / `turnPath` / `entityPaths`，见"场景"一节）加载并实例化到 `mapPos` 下，缓存 `MapManager` / `TurnManager` / `List<Entity>`，同时把地图引用发给实体、把实体物体列表接进地图 `entities`（顺序对应 `spawnPoints`）；`InitBattle()` 先订死亡消息，再依次 `MapManager.Init()` → 每个 `Entity.Init()` → `TurnManager.Init(TurnInitData)` → 按 `autoStart` 决定开打。**收到 `EntityDied` 时统计还活着的阵营：只剩一个（或全灭）就 `EndGame()`——让回合停手 + 发 `BattleEnded` 消息** | `void BuildBattle()`（三步走一遍，可反复调）、`void StartBattle()`（转发给回合管理器）、`void RebuildBattle()`（= 清场重来）、`void ClearBattle()`（步骤①：按 回合 → 实体 → 地图 调各组件的 `Clear()`，再销毁加载出来的对象并清缓存）、`bool LoadBattle()`（步骤②，私有，关键对象加载不到返回 false）、`void InitBattle()`（步骤③，私有）、`void OnEntityDied(BattleEvent)`（私有，收死亡消息）、`void EndGame(int winnerTeam)`（私有，结束游戏 + 发结束消息，-1 = 打平） |
| `Events/EventPipeline.cs` | **基础事件管线**：`BattleEventType`（`EntityDied` / `BattleEnded`）+ `BattleEvent`（type / entity / team，静态构造 `BattleEvent.EntityDied(...)` / `BattleEvent.BattleEnded(...)`）+ 静态收发器 `EventPipeline` | `void Send(BattleEvent)`、`void Subscribe(BattleEventType, Action<BattleEvent>)`、`void Unsubscribe(...)`、`void Clear()`（清场时清订阅） |
| `Components/Health.cs` | 生命值 + 阵营；**死亡逻辑单独在 `Die()`**（停用自己 + 发 `EntityDied` 事件），扣血流程只把血扣到 0 再交给它；初始化由 `Entity.Init` 调 `Init()` | `float maxHealth`、`int team`、`float Current`、`bool IsDead`、`void Init()`（记住 owner + 把当前值补满）、`void Clear()`（清理：回到未初始化状态）、`TakeDamage(float)`（见底 → `Die()`）、`void Die()`（同一条命只走一次） |
| `Managers/MapManager.cs` | 网格 + **两份地图数据**（`int[,] cells` 格子→uuid 二维图、`Dictionary<int,Vector2Int> positions` uuid→位置，另有 `byUuid` uuid→实体），外部只读；`TryMove`/`CancelMove`/`SyncOccupied`/`ReleaseEntity` 负责维护；**订阅死亡消息**：单位阵亡就放开它的格子 | `Init()`（由 `BattleManager` 调：建网格 + 摆实体 + 订死亡消息）、`Clear()`（清理：退订 + 放掉地图数据与实体列表，保留 `spawnPoints` 配置）、`SyncOccupied()`（重建 + uuid 查重）、`CanEnter(cell)`、`static Manhattan(a, b)`、`CellToWorld(col,row)` / `CellToWorld(cell)`、`WorldToCell(pos)`、`TryMove(from, step, out to)`（裁决 + 维护占用）、`FindPath(from, to, path)`（四方向等权 BFS）、`int Cols` / `int Rows`、常量 `Empty=0` / `Unknown=-1`；**私有**：`Build` / `ResetEntities` / `ReleaseEntity`（死亡消息触发）/ `UuidAt` / `EntityAt` / `EntityOf` / `IsOccupied` / `InBounds` / `SelfCheck` / `SelfCheckPath`（后两个右键组件菜单能跑） |
| `Managers/TurnManager.cs` | 回合管理（**只管开始，停止/复位统一由 `BattleManager` 调它的 `Clear()`**）。数据：战斗实体列表 `actors` / 行动栈 `ActionStack`（每回合按先攻重建）/ 回合状态 `State`；每回合逐个出栈行动（`yield return actor.TakeTurnRoutine()`），跑满 `totalRounds` 结束 | `List<Entity> actors`、`List<Entity> ActionStack`、`TurnState State`（Idle / Running / Finished）、`int CurrentRound`、`Entity CurrentActor`、`int StackLeft`、`bool IsFinished`、`void Init(TurnInitData)`（由 `BattleManager` 调）、`TurnInitData`（actors / totalRounds / turnDelay）、`void StartBattle()`（**只管开始**，已在跑就直接返回，不会自己停）、`void Clear()`（清理：停协程 + 清空行动栈与参战列表 + 状态归零，由 `BattleManager.ClearBattle` 调）、`int totalRounds`（`BuildActionStack` / `PopNext` 已收成私有，只由内部回合循环用）、`float turnDelay`、`bool autoStart` |
...
- **地图数据**：格子→uuid 二维图与 uuid→位置/实体 字典都由 `MapManager` 维护，外部只读；改数据只能走 `TryMove`（走一格，裁决 + 维护占用）或 `SyncOccupied`（按坐标整体重建），阵亡放开格子由内部的 `ReleaseEntity` 负责。查表用的 `UuidAt` / `EntityAt` / `EntityOf` / `IsOccupied` / `InBounds` 都是私有方法。
...
- **事件走 `EventPipeline`**（静态收发器）：发送方 `Send(BattleEvent...)`，订阅方 `Subscribe(类型, 处理函数)`；目前 `Health.Die()` 发 `EntityDied`，两个订阅者：`BattleManager.OnEntityDied`（统计存活阵营，结算后发 `BattleEnded`）和 `MapManager.OnEntityDied`（`ReleaseEntity()` 放开阵亡者占的格子）。订阅在各自的 `Init()` 里订、`Clear()` 里退订（`MapManager.Init` 是先退订再订，重复 Init 也不会订两遍），`BattleManager.ClearBattle()` 末尾再用 `EventPipeline.Clear()` 兜一道（静态表不能留旧订阅）。加新事件就在 `BattleEventType` 加一个值 + 在 `BattleEvent` 里加一个静态构造。
...
- `Health.team`：阵营就是个 int，没有仇恨表/友军保护；`Die()` 只做 `SetActive(false)` + 发消息，**放开格子交给 `MapManager`**——它收到死亡消息会 `ReleaseEntity()`（清格子 + 从 `positions`/`byUuid`/`entities` 里摘掉），所以阵亡单位不再挡路、也不会再被当成目标；尸体物体本身还在 `BattleManager` 的 `entities` 列表里（结算统计要用），清场时才销毁
