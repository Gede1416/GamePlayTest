...
- `Assets/Scripts/Buffs/`：Buff（`BuffManager` 只管自己的队列 / `BuffPipeline` 两条接口 + `BuffType` 名字 / `BuffEffect/` 效果零件 / `Combination/` 一个 buff 一个类）
...
| `Buffs/Combination/HealBuff.cs` / `AddMoveStepsBuff.cs` / `AddAttackBuff.cs` | **一条 buff = 一组效果零件拼起来**（和技能一个套路）：名字 / 数值 / 持续回合是各自的常量，`effects` 列表 + 目标（单个实体）+ 已结算次数是自己的状态，`Add` / `Trigger` / `Remove` 就是"对这个目标、每个零件跑一遍对应动作"；持续回合按**结算次数**算（`Duration <= 0` = 永久）。数值：治疗 20 × 3、步数 +2 × 3、攻击 +15 × 3 | `Init(Entity target)`（由 BuffManager 发下来）、`Add()` / `Trigger()` / `Remove()`、`BuffType Type`、`int Left`、`bool IsOver`、无参构造（`BuffCaster<T>` 要 `new()`） |
...
| `Pipeline/Skill/SkillCaster/DamageCaster.cs` / `BuffCaster.cs` | 阶段三：扣血（走 `Entity.TakeDamage` 门面，`damage` 能被 buff 加减）/ 挂 buff——**挂哪种由技能这边配**：`BuffCaster<T> where T : IBuff, new()`，泛型参数就是那份组合 buff（`BuffCaster<HealBuff>` 等），每次挂都新建一条 | `DamageCaster(float damage)`、`AddDamage(float)`、`BuffCaster<T>()`、`bool Cast(caster, targets)` |
...
- **Buff 也照技能那套拼**：`Buffs/` 的布局和 `Pipeline/Skill/` 一样——管理器（`BuffManager`，**只管自己身上这一队 buff**，不认名字也不造 buff）+ 接口文件（`BuffPipeline.cs`，`BuffType` 在这里，只当 buff 的名字）+ 零件目录（`BuffEffect/`，一个效果一个文件，数值走构造函数）+ 组合目录（`Combination/`，一个 buff 一个类，数值与持续回合是它的常量、目标与已结算次数是它的状态，`Init(target)` 挂在谁身上由管理器发下来）。`IBuff` 的三步（`Add` / `Trigger` / `Remove`）就是"对这个目标、每个效果零件跑一遍对应动作"；**挂哪种 buff 由技能那边挑**（`BuffCaster<HealBuff>`），**挂给谁由技能阶段二挑好**、逐个挂过来——没有 `Buff` / `BuffCfg` / `BuffFactory` 那套基础结构，也没有"按名字造 buff"。
