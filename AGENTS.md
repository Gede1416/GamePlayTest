...
- 场景已不是空场景：`entity` / `entity (1)` / `ground` / 4 个 `skill_*` / `TestCanvas` + 4 个按钮 / `EventSystem`，详见下面"场景"一节

## 脚本清单

- `Assets/Scripts/`：`Entity`（实体）、`NavTest`（测试用）
- `Assets/Scripts/Managers/`：管理器（`InitManager`（唯一挂场景的）/ `MapManager`（普通类）/ `TurnManager`（普通类）/ `SkillManager`）
...
| `Managers/MapManager.cs` | **普通类（不是 MonoBehaviour）**：网格 + 实体初始位置 + 格子占用（外部只读）；由 `InitManager` 构建 | `Init(entities, spawnPoints, map)`、`Build()`、`SyncOccupied()`、`ResetEntities()`、`CellToWorld()`、`WorldToCell()`、`InBounds()`、`IsOccupied()`、`CanEnter()`、`static Manhattan(a,b)`、`static GetBounds(go)`、`TryMove(from, step, out to)`、`CancelMove(from, to)`、`Cols`/`Rows`/`MinXZ`/`GridY`/`Occupied` |
| `Managers/TurnManager.cs` | **普通类（不是 MonoBehaviour）**：每回合按 `Entity.speed` 让每个角色行动一次，跑满 `totalRounds` 就结束；角色"行动" = `Entity.TakeTurn()` 并等 `Pilot.IsFollowing` 走完；`Run()` 返回 IEnumerator，由 InitManager 去 StartCoroutine | `Init(actors)`、`IEnumerator Run()`、`List<Entity> Order()`、`List<Entity> actors`、`int totalRounds`、`float turnDelay`、`int CurrentRound`、`Entity CurrentActor`、`bool IsFinished` |
| `Managers/InitManager.cs` | **唯一挂场景的管理器**：`Awake` 里 `new MapManager{cellSize}.Init(entities, spawnPoints, map)` + `new TurnManager{...}.Init(actors)`，并把地图注入 `ObjectMover` / `Entity` / `NavTest`；`Start` 起回合协程；网格与占用的 Gizmos 也在这里画 | `MapManager Map`、`TurnManager Turns`、`void StartBattle()`、`void StopBattle()`；Inspector 配置：`map`/`entities`/`spawnPoints`/`cellSize`/`actors`/`totalRounds`/`turnDelay`/`autoStart` |
| `Entity.cs` | 实体：一个角色身上组件的统一入口（`[RequireComponent]` ObjectMover + AutoPilot）；地图由 `InitManager` 经 `Setup(map)` 注入，之后用工厂按枚举装配管线三段 + 写步数上限；`SourceType` 改枚举即重建阶段一 | `AutoPilot Pilot`、`ObjectMover Mover`、`Health Health`、`int Team`、`int speed`（先攻）、`int moveSteps`（每回合步数上限，0=不限）、`TargetSourceType SourceType { get; set; }`、`void Setup(MapManager)`、`void Wire()`、`void ApplySteps()`、`bool TakeTurn()` |
...
- `ground`：MeshCollider + MeshRenderer + MeshFilter + **InitManager**（配置都挂这里：`map` = ground 自己、`cellSize` 1、`entities` = [entity, entity (1)]、`spawnPoints` = [(1,1), (5,5)]（格子坐标）、`actors` = 两个 Entity、`totalRounds` 5、`turnDelay` 0.2、`autoStart` 开）；地面 Plane 10×10 → Cols/Rows 10
- `entity`：Transform/MeshFilter/MeshRenderer + BoxCollider + ObjectMover（`map` 运行时由 InitManager 注入）+ **AutoPilot** + Health(`team: 0`) + SkillManager + **Entity(先攻 10, moveSteps 4)**；4 个 skill_* 是它的子物体（Rigidbody 已被用户在编辑器里删掉）
- `entity (1)`：BoxCollider + Health（`team: 1`，敌方）+ **ObjectMover + AutoPilot**（`mover` 已接好，`map` 运行时注入）+ **Entity(先攻 10, moveSteps 3, targetSourceType = FleeNearestEnemy)** ——**按用户要求不挂攻击/技能组件**（没有 Attack、没有 SkillManager）
...
- 回合配置现在在 `InitManager` 上（原来的 `TurnManager` 物体已删除）：`actors` = [entity 上的 Entity, entity (1) 上的 Entity]，先攻在各自 `Entity.speed` 上（都是 10 → 相同则按列表顺序，entity 先动），播放就自动跑 5 回合
...
- 需要可视化的逻辑（如网格划分）用 `OnDrawGizmosSelected` 画出来核对，不写单元测试（普通类画不了 Gizmos，所以网格/占用的绘制在 `InitManager` 上）。
- `MapManager` / `TurnManager` 是**普通类**，由 `InitManager` 在 `Awake` 里构建、并把地图注入给 `ObjectMover` / `Entity` / `NavTest`；组件里不要再写 `FindObjectOfType<MapManager>()`，要地图就从 `Entity` 或 `InitManager` 拿。
...
- `MapManager`：占用数据只按 `entities` 列表重建（不在列表里的实体会走但不会被记录占用）；`entities` 被搬动/销毁后要自己调 `SyncOccupied()` 对齐；无地形/障碍数据、无寻路；没有 MonoBehaviour 生命周期，构建与时机都交给 `InitManager`
...
- `TurnManager`：`StartBattle` / `StopBattle` 搬到了 `InitManager`（协程要挂在 MonoBehaviour 上）；行动内容写死成 `Entity.TakeTurn()`（跑一次寻路管线，没抽成可替换的行动接口）、没有回合开始/结束事件（只能轮询 `IsFinished`）、先攻相同时按列表顺序而不掷骰、没有"跳过/延后/守卫"这类规则
...
- `NavTest`：纯测试组件——按钮文字用英文（内置字体没有中文字形）、不管连点/换目标；`anchor` / `pilot` 在 Inspector 里接，`map` 由 `InitManager` 注入
