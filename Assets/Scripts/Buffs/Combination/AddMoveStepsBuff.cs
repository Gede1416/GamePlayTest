...
/// <summary>
/// 增加移动步数 buff：**一条 buff = 一组效果零件拼起来**（这个类就是那个组合）。
/// 挂上时步数 +2、移除时 -2（本回合预算也跟着走），3 个回合后到期。
...
public class AddMoveStepsBuff : IBuff
{
    ...
    {
        new MoveStepsEffect(MoveStepsBonus),
    };
    ...
    public BuffType Type => BuffType.AddMoveSteps;
    ...
    /// <summary>挂上：跑一遍各零件的 Apply（步数加成当场生效）</summary>
    public void Add()
    {
    ...
    /// <summary>移除：跑一遍各零件的 Revert（步数加成减回去）</summary>
    public void Remove()
    {
        if (target == null) return;
