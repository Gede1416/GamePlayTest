...
/// （挂哪个零件 + 数值 + 持续回合；原来 Buffs/Combination/HealBuff 那份职责搬到这里）。
...
public class HealBuffCaster : ISkillCaster
{
    ...
    public bool Cast(Entity caster, List<Entity> targets)
    {
        ...
        foreach (var t in targets)
            if (t != null) t.AddBuff(new HealEffect(HealPerTurn, Duration));

        Debug.Log($"[HealBuffCaster] {caster.name} 放下治疗 buff（{Duration} 回合）");
        return true;
    }
}
