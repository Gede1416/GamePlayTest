...
/// （效果零件 + 名字 + 持续回合；原来 Buffs/Combination/AddMoveStepsBuff 那份职责搬到这里）。
...
public class MoveStepsBuffCaster : ISkillCaster
{
    ...
    public bool Cast(Entity caster, List<Entity> targets)
    {
        ...
        foreach (var t in targets)
            if (t != null) t.AddBuff(new Buff(BuffType.AddMoveSteps, Duration, new MoveStepsEffect(MoveStepsBonus)));

        Debug.Log($"[MoveStepsBuffCaster] {caster.name} 放下 {BuffType.AddMoveSteps}");
        return true;
    }
}
